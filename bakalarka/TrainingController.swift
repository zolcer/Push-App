...
class TrainingController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        if (FBSDKAccessToken.current() != nil) {
            FBSDKGraphRequest(graphPath: "me", parameters: ["fields": "id, name, first_name, last_name, picture.type(large), email"]).start(completionHandler: { (connection, result, error) -> Void in
                if (error == nil){
                    ...
                    self.message.text=self.dict["first_name"] as? String
                    print("hababa")
                    print(self.dict)
                }
            })
        ...
        */
        ...
        timer = Timer.scheduledTimer(timeInterval: 1.0, target: self, selector: #selector(TrainingController.gyroutput), userInfo: nil, repeats: true)
